...
#x_vals = np.stack( ( normalize(x_orig[:,0]), normalize(x_orig[:,1]) ),axis=1)
...
#wxmin = tf.reduce_min(tf.matmul( wvec,tf.transpose(x_data) ))
#wxmax = tf.reduce_max(tf.matmul( wvec,tf.transpose(x_data) ))
...
prediction_output = tf.add( tf.matmul(tf.multiply(tf.transpose(y_target),b), pred_kernel), intercept) 
...
for i in range(6000): 
    ...
    #temp_wx_min = sess.run(wvec, feed_dict={x_data: rand_x, y_target: rand_y}) 
    
    #temp_wx_min = sess.run(wxmin, feed_dict={x_data: rand_x, y_target: rand_y}) 
    #temp_wx_max = sess.run(wxmax, feed_dict={x_data: rand_x, y_target: rand_y}) 
    temp_intercept = sess.run(intercept, feed_dict={x_data: rand_x, y_target: rand_y}) 
    acc_temp = sess.run(accuracy, feed_dict={x_data: rand_x, 
                                             y_target: rand_y, 
                                             prediction_grid:rand_x}) 
    #temp_index_minus = sess.run(indices_minus, feed_dict={x_data: rand_x, y_target: rand_y}) 
    #print(temp_index_minus)
